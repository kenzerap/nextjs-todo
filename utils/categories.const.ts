export const categories: { name: string; coverImg: string }[] = [
  {
    name: `Fashion`,
    coverImg:
      'https://static.nike.com/a/images/f_auto/dpr_1.0,cs_srgb/w_593,c_limit/46a63970-621c-454c-951b-da4a852be392/image.png',
  },
  {
    name: `Shoes`,
    coverImg:
      'https://static.nike.com/a/images/f_auto/dpr_1.0,cs_srgb/w_1808,c_limit/2c316733-7080-4fa9-84f9-c804fac9d053/nike-just-do-it.jpg',
  },
  {
    name: `Laptops`,
    coverImg:
      'https://images.inc.com/uploaded_files/image/1920x1080/getty_1208082286_427081.jpg',
  },
  {
    name: `Books`,
    coverImg:
      'https://images.theconversation.com/files/45159/original/rptgtpxd-1396254731.jpg?ixlib=rb-1.1.0&q=45&auto=format&w=754&h=502&fit=crop&dpr=1',
  },
  {
    name: `Toys`,
    coverImg: 'https://m.media-amazon.com/images/I/81tMKC00LLL._AC_SX679_.jpg',
  },
  {
    name: `Gaming accessories`,
    coverImg:
      'https://sm.mashable.com/t/mashable_in/photo/default/gaming-accessories-copy_suwm.1248.jpg',
  },
  {
    name: `Foods`,
    coverImg:
      'https://www.cnet.com/a/img/resize/69256d2623afcbaa911f08edc45fb2d3f6a8e172/hub/2023/02/03/afedd3ee-671d-4189-bf39-4f312248fb27/gettyimages-1042132904.jpg?auto=webp&fit=crop&height=675&width=1200',
  },
];
